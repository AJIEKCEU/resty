...
function init_product()
{
    ...
    $('.card .switcher_tabs label').click(function()
    {
        ...
        $(".card_price__month").text($(this).find("span").text());
...
jQuery(document).ready(function ($)
{
  ...
  /*  $("body").on("click", ".buy_with_resty, .buy_now, .form_radio.rassrochka", function()
    {
        setTimeout(function()
        {
            update_visible_prices();
        }, 50);
    });

    // --- отобразим нужную разбежку в ценах на кнопках
    function update_visible_prices()
    {
        // --- иначе покажем согласно методу рассрочки рести:
        var period_mes = $(".period_mes:checked").val();

        $(".full_price").removeClass('resty_12');
        $(".full_price").removeClass('resty_24');
        $(".full_price").addClass('resty_'+period_mes);
